...
async function starApp() {
  const dayOfAdminReport = 1;
  const isTodayForReportGeneration =
    AppUtil.isTodayNthDayOfMonth(dayOfAdminReport);
  if (isTodayForReportGeneration) {
    const monthIndex = new Date().getMonth();
    const startDate = AppUtil.getFormattedDate(
      new Date(new Date().setDate(dayOfAdminReport))
    );
    const endDate = AppUtil.getFormattedDate(
      new Date(
        new Date(new Date().setMonth(monthIndex, 0)).setDate(dayOfAdminReport)
      )
    );
